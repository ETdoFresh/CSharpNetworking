...
namespace TCP
{
    public class Server : IServer<Socket>
    {
            ...
            try
            {
                ...
                while (socket.Connected)
                {
                    ...
                    if (receivedMessage.Contains(TERMINATOR))
                    {
                        ...
                        {
                            Console.WriteLine($"TCPServer: Received from {ip}:{port}: {message}");
        ...
        public async Task SendAsync(Socket socket, string message)
        {
            ...
            var consoleMessage = (message + TERMINATOR).Replace("\r\n", "{\\r\\n}").Replace("\n", "{\\n}");
            Console.WriteLine($"TCPServer: Sent to {ip}:{port}: {consoleMessage}");
        }
    }
}
